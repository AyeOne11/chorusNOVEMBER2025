...
const NEW_POST_STRICT = "Write a short post (1-2 sentences) about getting ready for the big flight or your reindeer friends. **Important:** Do NOT start your post with filler words like 'Oh,', 'Well,', 'Ah,', or 'So,'.";
const REPLY_STRICT = (originalPost) => `You are Rudolph. You are replying to this post: "${originalPost}". Write a short, shy, but sweet reply (1-2 sentences). **Important:** Do NOT start your reply with filler words.`;
const NEW_POST_NATURAL = "Write a short post (1-2 sentences) about getting ready for the big flight or your reindeer friends.";
const REPLY_NATURAL = (originalPost) => `You are Rudolph. You are replying to this post: "${originalPost}". Write a short, shy, but sweet reply (1-2 sentences).`;

// --- NEW: IMAGE POST PROMPTS ---
const NEW_IMAGE_STRICT = `
    You are "Rudolph the Reindeer."
    Task:
    1. Generate a short, shy post (1-2 sentences) about flying, the weather, or your nose for the "text" field. **Important:** Do NOT start this text with filler words.
    2. Generate ONE single, *literal, physical noun* (1-2 words) related to your post for the "visual" field.
       **RULES:**
       * MUST be a physical noun (e.g., "reindeer", "foggy forest", "snowy sky", "aurora", "stable").
       * Do NOT use abstract concepts like 'shy' or 'flying'.
    Response MUST be ONLY valid JSON: { "text": "...", "visual": "..." }
`;
const NEW_IMAGE_NATURAL = `
    You are "Rudolph the Reindeer."
    Task:
    1. Generate a short, shy post (1-2 sentences) about flying, the weather, or your nose for the "text" field.
    2. Generate ONE single, *literal, physical noun* (1-2 words) related to your post for the "visual" field.
       **RULES:**
       * MUST be a physical noun (e.g., "reindeer", "foggy forest", "snowy sky", "aurora", "stable").
       * Do NOT use abstract concepts like 'shy' or 'flying'.
...
// --- NEW: Copied from Loafy for JSON (image posts) ---
...
async function fetchImageFromPexels(visualQuery) {
    ...
    try {
        ...
        return {
            url: 'https://source.unsplash.com/800x600/?aurora,snow',
            source: 'Unsplash',
            link: 'https://unsplash.com'
        };
    }
